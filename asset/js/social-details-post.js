...
function getDetailsPost(post_id)
{
    $.ajax({
        url: base_url + "post-specific-dtls.php",
        type: "POST",
        data: {
            post_id: post_id
        },
        ...
        success: function(data) {
            if (data.length === 0) {
                $("#content_posts_dtls").html('');
            } else {
                $("#content_posts_dtls").html('');
                data.forEach(function(post) {
                    $("#content_posts_dtls").append(renderPostDtls(post));
                    ...
                    // Toggle options menu
                    var ellipsisMenu = document.querySelector('.ellipsis-menu');
                    if (ellipsisMenu) {
                    ...
                    var editPostButton = document.getElementById('editPost');
                    if (editPostButton) {
                    ...
                    var closePostButton = document.getElementById('closePost');
                    if (closePostButton) {
        ...
        <div class="card">
            ...
            <div class="card-footer">
                <div class="interaction-row">
                    <i class="far fa-heart like-button-dtls" data-id="${post.post_id}"></i>
                ...
                <div class="comment-section">
                    ...
                    <div class="comment-input">
                        <input type="text" class="comment_user_dtls" data-id="${post.post_id}" placeholder="Add a comment...">
                        <button type="button" class="comment_btn_dtls" data-id="${post.post_id}">Post</button>
                    </div>
                </div>
            </div>
        </div>
...
$(document).on('click', '.comment_btn_dtls', function() {
    ...
    let commentInput = $(`.comment_user_dtls[data-id="${postId}"]`);
...
function fetch_comment(postId) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (totalComments > 2) {
                var toggle_button = `<div class="view-all-comments-dtls" id="viewAllComments_${postId}" data-id="${postId}" style="cursor:pointer;">
...
$(document).on('click', '.view-all-comments-dtls', function() {
...
$(document).on('click', '.like-button-dtls', function() {
    // Get the data-id from the clicked element
    var postId = $(this).data('id');
...
$(document).on('dblclick', '.img-post', function() {
    ...
    var likeButton = $('.like-button-dtls[data-id="' + postId + '"]');
